...
import { formatCurrency, Currency } from "@/lib/currencyUtils";
...
export const generateQuotationPDF = async (
    data: QUOTATIONS & { showAccountDetails?: boolean; items: QuotationItems[] }
) => {
    try {
        ...
        currentPage.drawText(`Quote No : ${data.quote_id}`, {
            x: leftColX,
            y: leftY,
            size: labelSize,
            font: helveticaBold,
        ...
        leftY -= leftSpacing;
        currentPage.drawText(`Date : ${formattedDate(data.created_on)}`, {
            x: leftColX,
            y: leftY,
            size: labelSize,
            font: helvetica,
        ...
        currentPage.drawText(`TO : ${customerName}`, {
            x: rightColX,
            y: rightY,
            size: labelSize,
            font: helveticaBold,
        ...
        if (data.contact_person) {
            currentPage.drawText(data.contact_person, {
                x: rightColX,
                y: rightY,
                size: labelSize,
                font: helvetica,
        ...
        if (data.customer_phone) {
            currentPage.drawText(data.customer_phone, {
                x: rightColX,
                y: rightY,
                size: labelSize,
                font: helvetica,
        ...
        if (data.customer_email) {
            currentPage.drawText(data.customer_email, {
                x: rightColX,
                y: rightY,
                size: labelSize,
                font: helvetica,
        ...
        let colAlign: ("center" | "left" | "right")[] = [
            "center",
            "left",
            "center",
            "center",
        ...
        headers.forEach((header, i) => {
            ...
            let startY =
                yPosition - tableHeaderHeight / 2 + (lines.length > 1 ? 4 : -3);

            lines.forEach((line) => {
                const textWidth = helveticaBold.widthOfTextAtSize(line, 9);
                let textX = currentX + 5;
                if (colAlign[i] === "center")
                    textX = currentX + width / 2 - textWidth / 2;
                else if (colAlign[i] === "right")
                    textX = currentX + width - textWidth - 5;
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            // tax_type_id mapping: 0: VAT, 1: TIEP, 2: NONE
            const vatAmount =
                data?.tax_type_id === 0 || data?.tax_type_id === 1
                    ? itemTotal * 0.18
                    : 0;
            ...
            const rowHeight = Math.max(
                20,
                productLines.length * 12 + rowHeightBuffer
            );
            ...
            rowData.forEach((text, colIndex) => {
                ...
                if (colIndex === 1) {
                    ...
                    let textX = cellX + 5;
                    if (colAlign[colIndex] === "center")
                        textX = cellX + width / 2 - textWidth / 2;
                    else if (colAlign[colIndex] === "right")
                        textX = cellX + width - textWidth - 5;
        ...
        summaryLines.forEach((line) => {
            ...
            const currentSize = line.highlight
                ? summaryFontSize + 2
                : summaryFontSize;
        ...
        if (companyData.payment_terms) {
            ...
            lines.forEach((line: string, index: number) => {
                ...
                const prefixWidth = Math.max(
                    helvetica.widthOfTextAtSize(fullPrefix, 9),
                    20
                );
                ...
                currentPage.drawText(fullPrefix, {
                    x: margin,
                    y: yPosition,
                    size: 9,
                    font: helvetica,
                ...
                bodyLines.forEach((bLine, i) => {
                    if (i > 0) moveDown(12);
                    currentPage.drawText(bLine.trim(), {
                        x: indentX,
                        y: yPosition,
                        size: 9,
                        font: helvetica,
        ...
        if (data?.showAccountDetails && companyData.account_details) {
            ...
            currentPage.drawText("Account Details", {
                x: margin,
                y: yPosition,
                size: 12,
                font: helveticaBold,
            ...
            details.forEach((det) => {
                if (det.val) {
                    ...
                    currentPage.drawText(`${det.label}: ${det.val}`, {
                        x: margin,
                        y: yPosition,
                        size: 9,
                        font: helvetica,
